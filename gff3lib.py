    ...
    if os.path.isfile(input_file):
        with open(input_file, "r") as file:
            ...
            for line in lines:
                num_columns = len(
                    line.strip().split("\t")
                )  # Split the first line into columns based on the delimiter
...
class GFF3:
    ...
    def __init__(
        self, chromosome, source, type, start, end, score, strand, phase, name
    ):
    ...
    def parse_gff3(input_file):
        ...
        with open(input_file, "r") as gff3:
            ...
            for line in lines:
                ...
                if not re.search(
                    comment, line
                ):  # Create a new GFF3 instance from the file's columns and append it to a list
                    cols = line.split("\t")
                    gff3_instance = GFF3(
                        cols[0],
                        cols[1],
    ...
    def sort_gff3(input_file):
        ...
        def custom_sort_key(element):
            return (
                element.chromosome,
                element.start,
                entry_type_order.get(element.type, float("inf")),
            )
    ...
    def gff2gtf(input_file):  ## convert gff3 to bed format
        ...
        for gff3_instance in gff3_instances:
            ...
            out = "\t".join(
                [
                    gff3_instance.chromosome,
                    gff3_instance.source,
                    gff3_instance.start,
                    gff3_instance.end,
                    gff3_instance.score,
                    gff3_instance.strand,
                    gff3_instance.phase,
                    gff3_instance.name,
                ]
            )
    ...
    def gff2bed(input_file):  ## convert gff3 to bed format
        gff3_instances = GFF3.sort_gff3(
            input_file
        )  # Get the list of instances from sort_gff3

        out = str()
        bed = []

        for gff3_instance in gff3_instances:
            if gff3_instance.type == "gene" or gff3_instance.type == "pseudogene":
    ...
    def extract_range(
        input_file, chromosome, start, end
    ):  ## extract range from gff3 file
        gff3_instances = GFF3.sort_gff3(
            input_file
        )  # Get the list of instances from sort_gff3
    ...
    def extract_genes(
        input_file, gene_list, coords=True
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Library for efficiently manipulating gff3 files."
    )
    parser.add_argument(
        "--gff3", type=str, help="GFF3 file that you would like to sort."
    )
    ...
    parser.add_argument(
        ...
        "--extract",
        action="store_true",
        help="Option to extract range from GFF3 file.",
    )
    parser.add_argument(
        "-s", "--sort", action="store_true", help="Option to sort the GFF3 file."
    )
    parser.add_argument(
        "-gtf",
        "--gtf",
        action="store_true",
        help="Option to convert GFF3 to GTF file format.",
    )
    parser.add_argument(
        "-l",
        "--gene_list",
        type=str,
        help="List with gene IDs that you would like to extract.",
    )
    parser.add_argument(
        "-c",
        "--coords",
        type=str,
        help="Option to extract bed-like features for selected genes.",
    )
    parser.add_argument(
        "-r",
        "--range",
    ...
    parser.add_argument(
        ...
        "--chromosome",
        type=str,
        help="Chromosome whose range you would like to extract.",
    )
    parser.add_argument(
        "-st",
        "--start",
        type=str,
        help="Start position of the range you would like to extract.",
    )
    parser.add_argument(
        "-end",
        "--end",
        type=str,
        help="End position of the range you would like to extract.",
    )
    ...
    if args.gff3:
        ...
        if args.sort:
            ...
            with open(f"{inp}_extracted_{args.start}_{args.end}.gff3", "w") as f:
                for out in GFF3.extract_range(
                    args.gff3, args.chromosome, args.start, args.end
                ):
                    print(out, file=f)
